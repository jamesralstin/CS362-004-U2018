...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case smithy:
		...
		result = playSmithy(currentPlayer, state, handPos);
    ...
    case minion:
		...
		result = playMinion(state, currentPlayer, handPos, choice1, choice2);
	
      return result;
		
    case steward:

		result = playSteward(state, currentPlayer, handPos, choice1, choice2, choice3);
    ...
    case sea_hag:
		...
		result = playSeaHag(currentPlayer, state);
...
int playSmithy(int currentPlayer, struct gameState *state, int handPos){
	...
	//+3 Cards
	for (i = 0; i < 2; i++)//creating bug by running loop twice instead of three times
	{
		drawCard(currentPlayer, state);
	}
